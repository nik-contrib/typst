...
fn main() -> eyre::Result<()> {
    ...
    LANGUAGES
        .par_iter()
        .try_for_each(|lang| -> eyre::Result<()> {
            let (git, rev) =
                find_lang(lang).ok_or_else(|| eyre::eyre!("no lang: {lang}"))?;
            let grammar_dir = format!("checkouts/tree-sitter-{lang}");

            let rev = match *lang {
                // This language does not have a tree-sitter.json file, which means
                // we can't generate WASM for it
                "comment" => "689be73775bd2dd57b938b8e12bf50fec35a6ca3".to_string(),
                _ => rev,
            };
            ...
            // Create empty files for the queries, so `include_str!()` compiles
            for query in ["highlights.scm", "locals.scm", "injections.scm"] {
                let query_path = PathBuf::from(format!(
                    "checkouts/helix/runtime/queries/{lang}/{query}"
                ));
        ...
        .wrap_err("failed to build language")?;
    ...
    writeln!(s, "std::collections::HashMap::from([")?;

    for lang in LANGUAGES {
        let checkouts_dir = format!("../../../../checkouts");
        let queries_dir = format!("{checkouts_dir}/helix/runtime/queries/{lang}");
        writeln!(
            s,
            r#"
    (
        "{lang}",
        LanguageData {{
            highlights_query: include_str!("{queries_dir}/highlights.scm"),
            injections_query: include_str!("{queries_dir}/injections.scm"),
            locals_query: include_str!("{queries_dir}/locals.scm"),
            wasm: include_bytes!("{checkouts_dir}/tree-sitter-{lang}/grammar.wasm"),
        }},
    ),"#
        )?;
    }
    ...
    writeln!(s, "pub static THEME: std::sync::LazyLock<")?;
    ...
    std::fs::write("crates/typst-library/src/text/tree_sitter_generated.rs", s)?;
