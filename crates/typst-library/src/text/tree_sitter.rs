...
use crate::{
    ...
    loading::{DataSource, Load as _, Loaded},
    visualize::Color,
};
...
impl TreeSitterHighlightConfiguration {
    ...
    pub(crate) fn decode(
        ...
        let mut errors = EcoVec::new();
        let highlights_query = match highlights_query.map(|q| q.data.as_str().within(q)) {
            Some(Ok(query)) => query,
            Some(Err(errs)) => {
                errors.extend(errs);
                ""
        ...
        let injections_query = match injections_query.map(|q| q.data.as_str().within(q)) {
            Some(Ok(query)) => query,
            Some(Err(errs)) => {
                errors.extend(errs);
                ""
        ...
        let locals_query = match locals_query.map(|q| q.data.as_str().within(q)) {
            Some(Ok(query)) => query,
            Some(Err(errs)) => {
                errors.extend(errs);
                ""
        ...
        let mut highlight_configuration =
            match tree_sitter_highlight::HighlightConfiguration::new(
                ...
                Ok(conf) => conf,
                Err(error) => {
                    errors.push(SourceDiagnostic::warning(
                        name.span,
                        format!(
        ...
        Ok(TreeSitterHighlightConfiguration {
            aliases: aliases.into(),
            config: Arc::new(ManuallyHash::new(highlight_configuration, lang_hash)),
        })
    }
